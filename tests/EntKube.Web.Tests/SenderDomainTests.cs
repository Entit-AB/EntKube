...
public class SenderDomainTests
{
    ...
    [Theory]
    [InlineData("anna@capio.se", "capio.se")]
    [InlineData("Anna.Lindqvist@CAPIO.SE", "capio.se")]
    [InlineData("  anna@capio.se  ", "capio.se")]
    [InlineData("anna@it.capio.se", "it.capio.se")]
    ...
    public void The_domain_is_what_follows_the_last_at() =>
        SenderDomain.Of("\"odd@name\"@capio.se").Should().Be("capio.se");

    // ---- What a registered domain claims ---------------------------------------------------

    [Fact]
    public void A_domain_claims_itself() =>
        SenderDomain.Claims("capio.se", "capio.se").Should().BeTrue();

    /// <summary>
    /// A customer saying "our mail comes from capio.se" means their subdomains too. Making
    ...
    public void A_domain_claims_its_subdomains() =>
        SenderDomain.Claims("capio.se", "it.capio.se").Should().BeTrue();

    /// <summary>
    /// <b>The one that matters.</b> Without the dot boundary, registering capio.se would
    /// claim notcapio.se — a domain anybody in the world can register, whose mail would
    /// then be triaged straight into that customer's queue.
    /// </summary>
    [Theory]
    [InlineData("notcapio.se")]
    [InlineData("xcapio.se")]
    [InlineData("capio.se.evil.com")]
    public void A_domain_does_not_claim_one_that_merely_ends_the_same(string sender) =>
        SenderDomain.Claims("capio.se", sender).Should().BeFalse();

    [Fact]
    public void A_subdomain_registration_does_not_claim_the_parent() =>
        SenderDomain.Claims("it.capio.se", "capio.se").Should().BeFalse();
    ...
    public void The_most_specific_registered_domain_wins()
    {
        ...
        [
            new("capio.se", "Capio"),
            new("it.capio.se", "Capio IT"),
        ];

        SenderDomain.BestMatch(registered, r => r.Domain, "it.capio.se")!
            .Customer.Should().Be("Capio IT");

        SenderDomain.BestMatch(registered, r => r.Domain, "hr.capio.se")!
            .Customer.Should().Be("Capio");
    }
    ...
    {
        Registered[] forwards = [new("capio.se", "Capio"), new("it.capio.se", "Capio IT")];
        Registered[] backwards = [.. forwards.Reverse()];

        SenderDomain.BestMatch(forwards, r => r.Domain, "it.capio.se")!.Customer
            .Should().Be(SenderDomain.BestMatch(backwards, r => r.Domain, "it.capio.se")!.Customer);
    }

    [Fact]
    public void Nothing_matches_when_no_domain_is_registered() =>
        SenderDomain.BestMatch<Registered>([], r => r.Domain, "capio.se").Should().BeNull();

    [Fact]
    public void Nothing_matches_a_sender_with_no_domain() =>
        SenderDomain.BestMatch([new Registered("capio.se", "Capio")], r => r.Domain, null)
            .Should().BeNull();

    // ---- Tidying what somebody typed ----------------------------------------------------------

    [Theory]
    [InlineData("capio.se", "capio.se")]
    [InlineData("  CAPIO.SE ", "capio.se")]
    [InlineData("@capio.se", "capio.se")]
    [InlineData("*.capio.se", "capio.se")]
    [InlineData(".capio.se", "capio.se")]
    [InlineData("https://capio.se/", "capio.se")]
    ...
    public void A_customers_own_domain_is_not_a_public_provider() =>
        SenderDomain.PublicProviders.Contains("capio.se").Should().BeFalse();
}
